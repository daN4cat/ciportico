    ...
    */
    ...
    function derive_ajax_operation() 
    {
        ...
        if ( !$this->url_path_to_reportico_runner )
            $this->url_path_to_reportico_runner = $this->reportico_url_path;
	...
	function initialize_panels($mode)
	{
		...
		$smarty = new smarty();
		$smarty->template_dir = find_best_location_in_include_path( $this->templates_folder );
		$smarty->compile_dir = find_best_location_in_include_path( $this->compiled_templates_folder );
	...
	function execute($mode=false, $draw=true)
	{
		...
		if ( !( is_dir ($this->compiled_templates_folder)) )
		{
			find_file_to_include($this->compiled_templates_folder, $include_template_dir, $include_template_dir);
		}
...
//echo "set table ";
		...
		switch ($mode) 
		{
			...
			case "PREPARE":
                ...
                restore_error_handler();
				if (preg_match("/$reportname/", find_best_location_in_include_path( $this->templates_folder . "/". $reportname )))
				{
